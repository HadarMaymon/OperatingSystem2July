...
namespace Scheduling
{
    class CPU
    {
        ...
        private int ExecuteLine(string sLine)
        {
            ...
            {
                //your code here HADAR
                //yield releases the CPU and activates the scheduler
                ProgramCounter++;  // Set remaining time to 0 to force a context switch on the next cycle.
                OperatingSystem.ActivateScheduler();  // Call scheduler to pick the next process.
                return ProgramCounter;  // Return the current program counter to resume correctly when rescheduled.
